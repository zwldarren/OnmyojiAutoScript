...
class HyakkiyakouConfig(ConfigBase):
    ...
    hya_sp: float = Field(default=1.0, description="hya_sp_help")
    hya_ssr: float = Field(default=1.0, description="hya_ssr_help")
...
class HyakkiyakouModels(ConfigBase):
    ...
    # 模型的推理精度 目前全部是fp32
    model_precision: ModelPrecision = Field(
        default=ModelPrecision.FP32, description="model_precision_help"
    )
    # 推理引擎 目前全部是onnxruntime
    inference_engine: InferenceEngine = Field(
        default=InferenceEngine.ONNXRUNTIME, description="inference_engine_help"
    )
...
class DebugConfig(ConfigBase):
    ...
    # 单独的截屏设置
    hya_screenshot_method: ScreenshotMethod = Field(
        default=ScreenshotMethod.WINDOW_BACKGROUND, description="hya_screenshot"
    )
    # 单独的点击
    hya_control_method: ControlMethod = Field(
        default=ControlMethod.WINDOW_MESSAGE, description="hya_control_method"
    )
