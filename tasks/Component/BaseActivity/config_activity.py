...
class GeneralClimb(ConfigBase):
    ...
    def get_single_limit_time(self, climb_type: str, default: timedelta = None) -> timedelta | None:
        """
        获取单局限制时间
    ...
    def valid_run_sequence(self):
        ...
        if not sequence_list or len(sequence_list) < 1:
            raise ValueError("run sequence cannot be empty")
        label_set = {
            field.replace("_limit", "") for field in self.model_fields if field.endswith("_limit")
        }
        for climb_type in sequence_list:
            if climb_type not in label_set:
                raise ValueError(
                    f"run sequence can only be one of {', '.join(label_set)}, now is {climb_type}"
                )
        return self
