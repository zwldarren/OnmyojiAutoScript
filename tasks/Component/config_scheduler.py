...
from tasks.Component.config_base import ConfigBase, DateTime, Time, TimeDelta


class Scheduler(ConfigBase):
    enable: bool = Field(default=False, description="enable_help")
    next_run: DateTime = Field(
        default=DateTime.fromisoformat("2023-01-01 00:00:00"), description="next_run_help"
    )
