...
from tasks.GameUi.page import page_shikigami_records, page_soul_zones
...
class ScriptTask(OrochiScriptTask, TrueOrochiAssets):
        ...
        while 1:
            ...
            if self.appear_then_click(self.I_ST_FIRE, interval=4):
                ...
                if current_success < 0 or current_success > 2:
                    continue
                logger.info(
                    f"current: {current}, current_success: {current_success}, total: {total}"
                )
    ...
    def check_times(self, battle: bool):
        ...
        if now_week_number != next_run_week_number:
            ...
            self.config.true_orochi.true_orochi_config.current_success = min(
                2, self.config.true_orochi.true_orochi_config.current_success
            )
        self.config.save()
