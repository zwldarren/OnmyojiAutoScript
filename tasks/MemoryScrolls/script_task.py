...
class ScriptTask(GameUi, MemoryScrollsAssets):
    ...
    def goto_memoryscrolls_main(self, con):
        ...
        if self.config.memory_scrolls.memory_scrolls_finish.auto_finish_exploration:
            ...
            if self.appear(self.I_MS_FRAGMENT_S_50):
                logger.info(
                    "Small Memory Scrolls fragments reached 50, planning tomorrow exploration"
                )
                # 安排下次探索
            ...
            self.ui_click_until_smt_disappear(
                self.I_MS_MAIN, stop=self.I_MS_FRAGMENT_S_VERIFICATION, interval=1.5
            )
        # 进入指定分卷
    ...
    def goto_scroll(self, con):
        ...
        if con.notification_95 and not self.appear(self.I_MS_COMPLETE_95):
            ...
            self.config.notifier.push(
                title="追忆绘卷进度95%", content="绘卷进度已达95%，请立即空降"
            )
    ...
    def contribute_memoryscrolls(self):
        ...
        while 1:
            self.screenshot()
            if (
                self.appear(self.I_MS_ZERO_S)
                and self.appear(self.I_MS_ZERO_M)
                and self.appear(self.I_MS_ZERO_L)
            ):
