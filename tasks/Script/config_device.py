...
class Device(BaseModel):
    ...
    package_name: PackageName = Field(
        title="Package Name", default=PackageName.AUTO, description="package_name_help"
    )
    screenshot_method: ScreenshotMethod = Field(
        default=ScreenshotMethod.AUTO, description="screenshot_method_help"
    )
    control_method: ControlMethod = Field(
        default=ControlMethod.MINITOUCH, description="control_method_help"
    )
    adb_restart: bool = Field(default=False, description="adb_restart_help")
    emulatorinfo_type: EmulatorInfoType | str = Field(
        default=EmulatorInfoType.AUTO, description="emulatorinfo_type_help"
    )
    ...
    run_background_only: bool = Field(
        default=False, description="模拟器无UI后台运行，关掉后重启脚本会重新显示（无需重启OAS）"
    )
