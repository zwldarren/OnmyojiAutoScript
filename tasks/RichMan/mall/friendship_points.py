...
class FriendshipPoints(Special):
    def execute_friendship(self, con: FriendshipPointsConfig = None):
        ...
        if con.white_daruma:
            self.buy_mall_one(
                buy_button=self.I_FS_WHITE_CLICK,
                buy_check=self.I_FS_WHITE_CHECK,
                money_ocr=self.O_MALL_RESOURCE_5,
                buy_money=1000,
            )
        if con.red_daruma != 0:
            self.buy_mall_more(
                buy_button=self.I_FS_RED,
                remain_number=False,
                money_ocr=self.O_MALL_RESOURCE_5,
    ...
    def buy_mall_one(
        self, buy_button: RuleImage, buy_check: RuleImage, money_ocr: RuleOcr, buy_money: int
    ):
        """
        针对只能买一个的
        :param buy_button:
