...
class Consignment(BaseModel):
    # 寄售屋
    enable: bool = Field(title="Enable", default=False)
    buy_sale_ticket: bool = Field(
        title="Buy Sale Ticket", default=False, description="buy_sale_ticket_help"
    )
...
class Scales(BaseModel):
    ...
    demon_class: DemonClass = Field(
        title="DemonClass", default=DemonClass.TSUCHIGUMO, description="demon_class_help"
    )
    demon_position: int = Field(
        title="Demon Position", default=1, description="demon_position_help"
    )
    picture_book_scrap: int = Field(
        title="Picture Book Scrap", default=30, description="picture_book_scrap_help"
    )
    picture_book_rule: str = Field(
        title="Picture Book Rule", default="auto", description="picture_book_rule_help"
    )
...
class SpecialRoom(BaseModel):
    ...
    medium_bondling_discs: int = Field(
        title="Medium Bondling Discs", default=0, description="medium_bondling_discs_special"
    )
    low_bondling_discs: int = Field(
        title="Low Bondling Discs", default=0, description="low_bondling_discs_special"
    )
...
class HonorRoom(BaseModel):
    # 杂货铺 荣誉购买
    enable: bool = Field(title="Enable", default=False)
    mystery_amulet: bool = Field(
        title="Mystery Amulet", default=False, description="mystery_amulet_help_honor"
    )
    black_daruma_scrap: bool = Field(
        title="Black Daruma Scrap", default=False, description="black_daruma_scrap_help_honor"
    )
...
class MedalRoom(BaseModel):
    ...
    white_daruma: bool = Field(title="White Daruma", default=False)
    challenge_pass: int = Field(
        title="Challenge Pass", default=0, description="challenge_pass_help"
    )
    red_daruma: int = Field(title="Red Daruma", default=0)
    broken_amulet: int = Field(title="Broken Amulet", default=0)
...
class Bondlings(BaseModel):
    ...
    bondling_stone: int = Field(
        title="Bondling Stone", default=0, description="bondling_stone_help"
    )
    high_bondling_discs: int = Field(
        title="High Bondling Discs", default=0, description="high_bondling_discs_help"
    )
    medium_bondling_discs: int = Field(
        title="Medium Bondling Discs", default=0, description="medium_bondling_discs_help"
    )
