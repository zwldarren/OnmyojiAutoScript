...
class Scrolls(BaseModel):
    ...
    scrolls_cd: TimeDelta = Field(
        title="间隔时间",
        default=TimeDelta(hours=0, minutes=30, seconds=0),
        description="scrolls_cd_help",
    )
    scrolls_threshold: int = Field(
        title="突破票数量", default="25", description="scrolls_threshold_help"
    )
...
class ExplorationConfig(BaseModel):
    ...
    exploration_level: ExplorationLevel = Field(
        title="探索等级",
        default=ExplorationLevel.EXPLORATION_28,
        description="exploration_level_help",
    )

    auto_rotate: AutoRotate = Field(
        title="自动添加候补式神", default=AutoRotate.no, description="auto_rotate_help"
    )

    choose_rarity: ChooseRarity = Field(
        title="选择狗粮稀有度", default=ChooseRarity.N, description="choose_rarity_help"
    )
