...
from tasks.MysteryShop.config import ShareConfig, ShopConfig
...
class ScriptTask(FriendshipPoints, MysteryShopAssets, GeneralInvite):
    ...
    def run_shop(self, shop_config: ShopConfig = None):
        ...
        if shop_config.mystery_amulet:
            while self.buy_mall_one(
                buy_button=self.I_MS_BLUE,
                buy_check=self.I_MS_CHECK_BLUE,
        ...
        if shop_config.black_daruma_scrap:
            while self.buy_mall_one(
                buy_button=self.I_MS_BLACK,
                buy_check=self.I_MS_CHECK_BLACK,
                money_ocr=self.O_MALL_RESOURCE_5,
                buy_money=60,
            ):
                pass
        if shop_config.shop_kaiko_3:
            while self.buy_mall_one(
                buy_button=self.I_MS_TAIKO_3,
                buy_check=self.I_MS_CHECK_TAIKO_3,
                money_ocr=self.O_MALL_RESOURCE_5,
                buy_money=45,
            ):
                pass
        if shop_config.shop_kaiko_4:
            while self.buy_mall_one(
                buy_button=self.I_MS_TAIKO_4,
                buy_check=self.I_MS_CHECK_TAIKO_4,
                money_ocr=self.O_MALL_RESOURCE_5,
                buy_money=80,
            ):
                pass
    ...
    def invite_friend(self, name: str = None, find_mode: FindMode = FindMode.AUTO_FIND) -> bool:
        ...
        logger.info("Now find friend in same server")
        selected = select(name)

        if not selected:
            self.ui_click_until_disappear(self.I_FLAG_2_OFF)
            logger.info("Now find friend in different server")
            selected = select(name)
    ...
    def next_time(self, success: bool = True):
        ...
        target_time: timedelta = timedelta(
