...
class ScriptTask(GeneralBattle, GameUi, SwitchSoul, AbyssShadowsAssets):
    ...
    def run(self):
        ...
        if today not in [4, 5, 6]:
            logger.info("Today is not abyss shadows day, exit")
            # 设置下次运行时间为本周五
        ...
        if cfg.abyss_shadows_combat_time.CombatTime_enable:
            while True:
                ...
                if self.boss_fight_count < 2:
                    self.boss_fight_count = self.fight_and_switch(
                        EmemyType.BOSS, 2, self.boss_fight_count, lambda: self.switch_area()
                    )

                # 2. 攻打 2 个 GENERAL
                if self.general_fight_count < 4:
                    self.general_fight_count = self.fight_and_switch(
                        EmemyType.GENERAL, 4, self.general_fight_count, lambda: self.switch_area()
                    )

                # 3. 攻打 3 个 ELITE
                if self.elite_fight_count < 6:
                    self.elite_fight_count = self.fight_and_switch(
                        EmemyType.ELITE, 6, self.elite_fight_count, lambda: self.switch_area()
                    )
        ...
        if success:
            ...
            if today == 4:
                # 周五推迟到周六
                logger.info("The next abyss shadows day is Saturday")
                self.custom_next_run(
                    task="AbyssShadows",
                ...
                # 周六推迟到周日
                logger.info("The next abyss shadows day is Sunday")
                self.custom_next_run(
                    task="AbyssShadows",
                ...
                # 周日推迟到下周五
                logger.info("The next abyss shadows day is Friday")
                self.custom_next_run(
                    task="AbyssShadows",
                    custom_time=cfg.abyss_shadows_time.custom_run_time_friday,
                    time_delta=5,
