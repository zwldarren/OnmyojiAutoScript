...
class ScriptTask(GameUi, CollectiveMissionsAssets):
    ...
    def detect_best(self) -> tuple:
        ...
        logger.info(
            f"first_class: {first_class}, second_class: {second_class}, third_class: {third_class}"
        )
        logger.info(
            f"first_order: {first_order}, second_order: {second_order}, third_order: {third_order}"
        )
    ...
    def _donate(self, index: int):
        ...
        max_index = 0
        max_number = 0
        for i, ocr in enumerate(
            [self.O_CM_1_MATTER, self.O_CM_2_MATTER, self.O_CM_3_MATTER, self.O_CM_4_MATTER]
        ):
