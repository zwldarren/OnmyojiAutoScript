...
class BestDemonBossSelect(BaseModel):
    best_demon_kiryou_utahime_select: bool = Field(
        title="Best Kiryou Utahime Select", default=False, description="极鬼灵歌姬是否开启"
    )
    best_demon_shinkirou_select: bool = Field(
        title="Best Shinkirou Select", default=False, description="极蜃气楼是否开启"
    )
    best_demon_tsuchigumo_select: bool = Field(
        title="Best Tsuchigumo Select", default=False, description="极土蜘蛛是否开启"
    )
    best_demon_gashadokuro_select: bool = Field(
        title="Best Gashadokuro Select", default=False, description="极荒骷髅是否开启"
    )
    best_demon_namazu_select: bool = Field(
        title="Best Namazu Select", default=False, description="极地震鲇是否开启"
    )
    best_demon_oboroguruma_select: bool = Field(
        title="Best Oboroguruma Select", default=False, description="极胧车是否开启"
    )
    best_demon_nightly_aramitama_select: bool = Field(
        title="Best Nightly Aramitama Select", default=False, description="极夜荒魂是否开启"
    )

    hide_fields = dynamic_hide(
        "best_demon_kiryou_utahime_select",
        "best_demon_oboroguruma_select",
...
class BestDemonConfig(BaseModel):
    ...
    hide_fields = dynamic_hide(
        ...
        "best_demon_oboroguruma",
        "best_demon_nightly_aramitamau",
    )


def convert_to_general_battle_config(
    boss_type: str,
    demon_battle_conf: "DemonBattleConfig" = None,
    best_demon_battle_conf: "BestDemonBattleConfig" = None,
) -> GeneralBattleConfig:
    ...
    return GeneralBattleConfig(
        preset_enable=enable, preset_group=group if enable else 1, preset_team=team if enable else 1
    )
...
class BestDemonBattleConfig(BaseModel):
    ...
    best_demon_nightly_aramitama_enable: bool = Field(
        default=False, description="是否切换极夜荒魂预设"
    )
    best_demon_nightly_aramitamau: str = Field(default="-1,-1", description="极夜荒魂预设")

    hide_fields = dynamic_hide(
        "best_demon_oboroguruma_enable",
        "best_demon_oboroguruma",
        "best_demon_nightly_aramitama_enable",
        "best_demon_nightly_aramitamau",
    )
