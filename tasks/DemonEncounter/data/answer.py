    ...
    def answer_one(self, question: str, options: list[str]) -> int | None:
                ...
                for value in values:
                    for index, option in enumerate(ops):
                        ...
                        if option_match_ratio >= 0.5:
                            ...
                            logger.warning(
                                "Now traversing the entire database to find the most similar question and option"
                            )
                            logger.warning(
                                f"Most similar question: {key} and most similar option: {value}"
                            )
                            return index + 1
            return None
        ...
        def decide_by_options(question: str, ops: list[str]):
            ...
            if matches:
                ...
                logger.warning(
                    f"Most similar answer: {opts[0][0]}, and similarity char number is {opts[0][1]}"
                )
                return opts[0][0]
        ...
        if opts:
            ...
            logger.warning(
                f"Use SequenceMatcher and get answers: {opts[0][0]}, score: {opts[0][1]}"
            )
            return opts[0][0]
