...
class UtilizeScheduler(Scheduler):
    ...
    success_interval: TimeDelta = Field(
        default=TimeDelta(hours=6), description="success_interval_help"
    )
    failure_interval: TimeDelta = Field(
        default=TimeDelta(hours=6), description="failure_interval_help"
    )
...
class UtilizeConfig(BaseModel):
    ...
    select_friend_list: SelectFriendList = Field(
        default=SelectFriendList.SAME_SERVER, description="select_friend_list_help"
    )
    shikigami_class: ShikigamiClass = Field(
        default=ShikigamiClass.N, description="shikigami_class_help"
    )
    shikigami_order: int = Field(default=4, description="shikigami_order_help")
    harvest_guild_max_times: int = Field(
        default=3, description="收取寮资金或体力失败的最大尝试次数"
    )
