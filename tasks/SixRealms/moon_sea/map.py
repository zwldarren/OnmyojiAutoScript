    ...
    def decide(self) -> tuple:
        ...
        for result in results:
            ...
            if x1 < rx1 or x2 > rx2 or y1 < ry1 or y2 > ry2:
                continue
            if isl_type == MoonSeaType.island100 and self.contains_any_char(
                result.ocr_text, chars="宁息"
            ):
                # 如果身上没有达到300块钱就不去了
                ...
                isl_roi = x1, y1, w, h
            elif isl_type == MoonSeaType.island100 and self.contains_any_char(
                result.ocr_text, chars="神秘"
            ):
                isl_type = MoonSeaType.island102
                isl_roi = x1, y1, w, h
            elif isl_type == MoonSeaType.island100 and self.contains_any_char(
                result.ocr_text, chars="回混范"
            ):
                isl_type = MoonSeaType.island103
                isl_roi = x1, y1, w, h
            elif isl_type == MoonSeaType.island100 and self.contains_any_char(
                result.ocr_text, chars="蜜馨屡战"
            ):
                isl_type = MoonSeaType.island104
                isl_roi = x1, y1, w, h
            elif isl_type == MoonSeaType.island100 and self.contains_any_char(
                result.ocr_text, chars="星之"
            ):
                isl_type = MoonSeaType.island105
                isl_roi = x1, y1, w, h
        logger.info(f"Island type: {isl_type}, Residue: {isl_num}, ROI: {isl_roi}")
