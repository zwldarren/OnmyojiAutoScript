...
class EmulatorManager(EmulatorManagerBase):
    ...
    def iter_uninstall_registry():
        ...
        for path in known_uninstall_registry_path:
            ...
            for software in software_list:
                ...
                try:
                    with winreg.OpenKey(
                        winreg.HKEY_LOCAL_MACHINE, f"{path}\\{software}"
                    ) as software_reg:
    ...
    def all_emulators(self) -> list[Emulator]:
        ...
        for uninstall in EmulatorManager.iter_uninstall_registry():
            ...
            for file in iter_folder(
                abspath(os.path.join(os.path.dirname(uninstall), "../")), ext=".exe"
            ):
                if Emulator.is_emulator(file) and os.path.exists(file):
                    exe.add(file)
            # MuMu specific directory
            for file in iter_folder(
                abspath(os.path.join(os.path.dirname(uninstall), "EmulatorShell")), ext=".exe"
            ):
                if Emulator.is_emulator(file) and os.path.exists(file):
                    exe.add(file)
