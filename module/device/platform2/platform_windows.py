...
class PlatformWindows(PlatformBase, EmulatorManager):
    ...
    def execute(cls, command, show_window=True):
        ...
        return subprocess.Popen(command, close_fds=True, startupinfo=startupinfo)
    ...
    def _emulator_start(self, instance: EmulatorInstance):
        ...
        if instance == Emulator.MuMuPlayer:
            ...
            self.execute(
                f'"{Emulator.single_to_console(exe)}" launch --index {instance.LDPlayer_id}',
    ...
    def emulator_start_watch(self):
        ...
        if self.config.script.device.run_background_only:
            ...
            logger.warning(
                "run_background_only will not show any UI, emulator will run background only"
            )
