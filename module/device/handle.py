...
class Handle:
    ...
    def __init__(self, config) -> None:
        ...
        logger.info(
            f"The root handle title is {self.root_handle_title} and num is {self.root_handle_num}"
        )
    ...
    def auto_handle_title(cls, windows: list) -> str:
        ...
        if len(emu_list) > 1 and emulator_title == "":
            logger.warning(
                f"Find more than one emulator handle, oas will use the first one {emu_list[0]}"
            )
    ...
    def emulator_family(self) -> EmulatorFamily:
        ...
        if children_num == 1:  #
            ...
            if name == "MuMuPlayer" or name == "NemuPlayer":
